...
function (pP) 
	...
	splitLineage <- function (prnt, off, nxtCd, ltCd)
		...
		sp1 <- data.frame(nm=as.character(prnt$nm),pn=as.character(prnt$pn),st=prnt$st,en=off$st,cd=prnt$cd)
		sp2 <- data.frame(nm=splName,pn=sp1$nm,st=off$st,en=prnt$en,cd=nxtCd)
		sp3 <- data.frame(nm=off$nm,pn=sp1$nm,st=off$st,en=off$en,cd=off$cd)
		spL <- data.frame(rbind(sp1,sp2,sp3))
		}
	
	if(class(pP)!="paleoPhylo") stop("object is not of class 'paleoPhylo'")
		{
		dat <- data.frame(nm=pP$nm,pn=pP$pn,st=pP$st,en=pP$en,cd=1:length(pP$nm))
		Nms <- data.frame(nms=as.character(pP$nm),st=pP$st)  ;  nms <- as.character(Nms[rev(order(pP$st)),1])

		for (k in 1:length(unique(dat$nm)))
			{
			dat$pn <- as.character(dat$pn)  ;  dat$nm <- as.character(dat$nm)
			...
			if (length(off[,1]) != 0)
				...
				for (i in 1:length(evnts))
					...
					if (sum(off$st == evnts[i]) == 2) 
						...
						for (k in 1:length(off1$nm)) {dat$pn[which(as.character(dat$nm) == as.character(off1$nm[k]))] <- as.character(prnt$nm)}
						}
		...
		YY <- with(dat,as.paleoPhylo(nm,pn,st,en))
		return(YY)
		}	
	}

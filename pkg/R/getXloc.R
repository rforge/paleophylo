getXloc <- function (pP) 
{
        ...
        stop("object is not of class 'paleoPhylo'")
    {
        ...
        for (j in 1:nn) dex[j] <- sum(sapply(1:nn, function(k) pP$nm[j] %in% 
            nAncs[[k]]))
        while (time != max(tmFrm)) {
            time <- tmFrm[(round(tmFrm, 8) > round(time, 8)) == 
                TRUE][1]
            event <- dat[which(-round(dat$st, 5) == round(time, 
                5)), ]
            parents <- unique(as.character(event$pn))
            inIds <- sapply(1:length(parents), function(i) sum(intersect(ids, 
                parents) == as.list(parents)[[i]]) > 0)
            parents <- c(parents[inIds == TRUE], parents[inIds == 
                FALSE])
            for (i in 1:length(parents)) {
                singleEvent <- event[as.character(event$pn) == 
                  parents[i], ]
                if (length(singleEvent[, 1]) > 1) 
                  evnt <- 1
                if (length(singleEvent[, 1]) == 1) 
                  evnt <- 0
                ...
                if (time != minTime) {
                  ...
                  if (length(pntLoc) == 0 & length(pos) > 1) 
                    stop(paste("There is a lack of congruence in the tree around", 
                      focInd, "\nHave a look at the ancestor", 
                      fcPrnt, "but also the sister species", 
                      fcSist[1], "and", fcSist[2], "\n"))
                }
                ...
                if (evnt == 1 & lnfi > 4 & lnsp == 1) 
                  stop("Cannot yet handle polytomies of order > 4 immediately post root.")
                if (evnt == 1 & lnfi == 3 & lnsp == 1) 
                  pos <- c(0.5, 0, 1, 0.25)
                if (evnt == 1 & lnfi == 4 & lnsp == 1) 
        ...
        if (length(locDat$xx) != length(unique(locDat$xx))) 
            stop("non-unique locations")

...
function(pP, begin=max(pP$st), end=min(pP$en), bin.length = 1){
	...
	if (end >= begin)
		stop("end must be later than begin!")
		
	if (bin.length <= 0)
		stop("bin.length must be positive!")
	
	cb <-createBifurcate(pP) #Can't be sure the paleoPhylo object passed will be bifurcating, so make it so	
	df <- with(cb, data.frame(nm, pn, st, en, xx, label, grp)) #Convert to data frame
	bin.start <- seq(from = begin, to = end+bin.length, by = -bin.length)
	rwb <- data.frame(bin.start)
	rwb$mu <- rwb$lambda <- rwb$extinctions <- rwb$originations <- rwb$branch.length <- rwb$N.at.start <- rep(NA, length(rwb$bin.start))
	...
	for (i in 1:length(rwb$bin.start)){
		...
		whoDoesSomething <- cb$nm[cb$en <= bin.start[i] & cb$en > bin.start[i]-bin.length]
		#Work out which of these disappear because they originate new lineages
		newLineages <- cb$nm[cb$st <= bin.start[i] & cb$st > (bin.start[i]-bin.length)]
		...
		whoOriginates <- setdiff(whoOriginates, given) #Don't count origin of first lineage in createBifurcate object
		...
		cb.copy <- cb
		cb.copy$st <- pmin(cb.copy$st, bin.start[i])
		cb.copy$en <- pmax(cb.copy$en, bin.start[i]-bin.length)
		bl <- pmax(0, cb.copy$st - cb.copy$en)
		rwb$branch.length[i] <- sum(bl)
		
		#Find diversity at start of bin
		rwb$N.at.start[i] <- length(prune.to.date(cb, bin.start[i], let.speciate=FALSE, let.die=FALSE)$phylo.tree$tip.label)
	}

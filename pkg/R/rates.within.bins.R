...
function(pP, begin=max(pP$st), end=min(pP$en), bin.length = 1){
	...
	for (i in 1:length(rwb$bin.start))
		...
		##should it not be length(cb$nm[cb$st>=bs & cb$en<bs])?
		rwb$N.at.start[i] <- length(cb$nm[cb$st>=bs & cb$en<bs])
		#length(prune.to.date(cb, bs, let.speciate=FALSE, let.die=FALSE)$phylo.tree$tip.label)
	}
